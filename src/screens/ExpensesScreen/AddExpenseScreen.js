...
const AddExpenseScreen = ({navigation}) => {
  ...
  const [expense_status, setExpenseStatus] = useState(
  ...
  const [expenses_report_title_error, setExpensesReportTitle_error] =
    useState(false);
  ...
  useEffect(() => {
    listCandidateJobs(user?.account_id, user?.candidate_id, '2')
      .then(response => {
        setJobs(response.data.data);
        if (response.data.data.length === 1) {
          set_selected_job(response.data.data[0].job_id);
          fun_set_selected_job(
            response.data.data,
  ...
  const Submit = is_draft => {
    ...
    if (
      expenses_report_title === null ||
      expenses_report_title.trim().length < 1
    ) {
      error = true;
        ...
        <SafeAreaView style={commonStyles.container}>
          ...
          <ScrollView
            ...
            <View style={{marginTop: scale(10)}}>
              <Text style={{...styles.label, marginLeft: scale(5)}}>
                Select Job
              </Text>
              ...
              {selected_job_error && (
                <Text style={{...textStyles.errorText, marginLeft: scale(5)}}>
                  Please select job
                </Text>
              )}
              <Spacer height={scale(3)} />
            ...
            {expense_list.map((item, index) => {
              return (
                <View key={`${index}`} style={styles.cardView}>
                  ...
                  <View>
                    <View>
                      <Text style={styles.label}>Expense Bill type</Text>
                      <Spacer height={scale(5)} />
                      <Select
                  ...
                  <View style={styles.Row}>
                    ...
                    <TouchableOpacity
                      ...
                      <Entypo name={'cross'} color={'#fff'} size={scale(40)} />
                    </TouchableOpacity>
                  </View>
                  {item.filepath_error && (
                    <Text style={textStyles.errorText}>
